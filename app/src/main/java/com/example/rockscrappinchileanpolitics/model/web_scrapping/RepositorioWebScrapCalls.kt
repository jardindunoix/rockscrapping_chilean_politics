...
class RepositorioWebScrapCalls { companion object {
	
	/*COMUNAL*/
	fun getConsejalesActualesDetail():MutableList<ConsejalActualDetalleEntity> {
		val comunasList:MutableList<ConsejalActualDetalleEntity> = mutableListOf()
		val url = StaticStrigns.URL_COMUNA_DETALLE + "arica"
			...
			if (element != h3Elements[0] && element.isNotEmpty() && isAllCaps(element)) {
				comunasList.add(
					ComunaConsejalActualEntity(
						nombre = element, paginaWeb = "${StaticStrigns.URL_COMUNA_DETALLE}${element}"
					)
				)
			}
		}
	...
	fun getDiputadosActuales():MutableList<DiputadoActualEntity> {
		...
		val imagesList = articleElement.select(StaticStrigns.IMG).eachAttr(StaticStrigns.SRC).stream()
			.collect(Collectors.toList()) as ArrayList
		var countAttr = 0
		var countName = 0
			...
			for (f in imagesList) {
				...
				diputadosActualesList.add(
					DiputadoActualEntity(
						nombre = name, paginaWeb = pagina, picture = image
					)
				)
				countAttr += 3
				++ countName
