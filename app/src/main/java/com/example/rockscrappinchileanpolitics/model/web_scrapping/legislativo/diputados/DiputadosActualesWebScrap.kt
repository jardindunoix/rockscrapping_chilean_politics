...
class DiputadosActualesWebScrap { internal class LoadInitNews():
	...
	override fun doInBackground(vararg params:Void?):ArrayList<DiputadoActualEntity> {
		try {
			...
			val nameList =
				h4Elements.stream().collect(Collectors.toList()) as ArrayList
			...
			if (webpage.size / 3 == nameList.size) {
				...
				for (f in imagesList) {
					...
					pagina =
						"${URL_DIPUTADOS_ACTUALES_ROOT}${DIPUTADOS}${webpage[countAttr]}"
					image = "${URL_DIPUTADOS_ACTUALES_ROOT}${f}"
					diputadosActualesList.add(
						DiputadoActualEntity(nombre = name, paginaWeb = pagina,
							picture = image))
					countAttr += 3
					++ countName
				}
